...
import id.xxx.submission.utils.DataMapper.listTvResultWithGenreToListTvResultModel
...
class TvRepository private constructor(
    ...
    companion object {
        ...
        fun getInstance(
            executors: Executors, remoteData: RemoteDataSource, localData: LocalTvDataSource
        ): TvRepository =
            instance ?: synchronized(this) {
                instance ?: TvRepository(executors, localData, remoteData)
            }
    }
    ...
    override fun getResult(supportSQLiteQuery: SupportSQLiteQuery?) =
        object : NetworkBoundResource<PagedList<TvResultModel>, TvResponse>(executors) {
            override fun loadFromDB(): LiveData<PagedList<TvResultModel>> {
                ...
                val convert =
                    result?.mapByPage { listTvResultWithGenreToListTvResultModel(it) }
                return convert?.let {
